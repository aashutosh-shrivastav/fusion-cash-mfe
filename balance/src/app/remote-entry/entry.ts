...
@Component({
  ...
  template: `
    <div style="padding: var(--fc-spacing-xl); background-color: var(--fc-color-surface-container); border: 1px solid var(--fc-color-outline-variant); border-radius: var(--fc-radius-lg); box-shadow: var(--fc-shadow-md);">
      <h2 style="font-family: var(--fc-font-family-heading); color: var(--fc-color-on-surface); font-size: var(--fc-font-size-2xl); margin-bottom: var(--fc-spacing-md);">Balance Service MFE</h2>
      <p style="font-family: var(--fc-font-family-base); color: var(--fc-color-on-surface-variant); font-size: var(--fc-font-size-md);">This is the Balance Service remote application loaded successfully!</p>
      ...
      <button (click)="fetchData()" style="margin-top: var(--fc-spacing-lg); padding: var(--fc-spacing-sm) var(--fc-spacing-md); background-color: var(--fc-color-primary); color: var(--fc-color-on-primary); border: none; border-radius: var(--fc-radius-md); font-family: var(--fc-font-family-base); font-weight: var(--fc-font-weight-medium); cursor: pointer; transition: background-color var(--fc-transition-fast);">
...
export class RemoteEntryComponent {
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/api/balance/data');
